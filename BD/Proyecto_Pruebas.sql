...
USE PROYECTO;
CREATE OR REPLACE ROLE 'Admin'GRANT 'Admin' TO;
GRANT ALL PRIVILEGES ON PROYECTO.* 'RR1998'@'%';
...
ALTER TABLE SUCURSAL ADD FOREIGN KEY (IDTelefono) REFERENCES TELEFONO(IDTelefono);

##Cliente
ALTER TABLE CLIENTE ADD FOREIGN KEY (IDPersona) REFERENCES PERSONA(IDPersona);
ALTER TABLE CLIENTE ADD FOREIGN KEY (IDEmpresa) REFERENCES EMPRESA(IDEmpresa);
...
ALTER TABLE PRODUCTO ADD FOREIGN KEY (IDCompra) REFERENCES COMPRA(IDCompra);
ALTER TABLE PRODUCTO ADD FOREIGN KEY (IDVenta) REFERENCES VENTA(IDVenta);
##Proveedor
...
ALTER TABLE PROVEEDOR ADD FOREIGN KEY (IDProducto) REFERENCES PRODUCTO(IDProducto);
...
##TRIGGERS
##Presona
CREATE TRIGGER VALIDARPERSONA BEFORE INSERT ON PERSONA
  FOR EACH ROW IF (SELECT DUI FROM PERSONA WHERE DUI = NEW.DUI) IS NOT NULL THEN RETURN 'Esa persona ya esta registrada';
  ELSE RETURN 'OK';
END IF;
